...
void Main()
{
	for(var verb = 0; verb != 100; ++verb){
		for(var noun = 0; noun != 100; ++noun){
			Int32[] ram = input.ToList().ToArray();// get fresh copy
			Init(ram, noun, verb);
			ProcessStep(ram, 0);
			if(ram[0]==19690720)
			{
				(noun*100+verb).Dump("answer");
			}
		}
	}
}
...
void ProcessStep(Int32[] ram, Int32 ip)
{
	while (ip < ram.Length && ip >= 0)
	{
		...
		{
			case 1:
				var add_lhs_pos = ram[ip++];
				var add_lhs = ram[add_lhs_pos];
				var add_rhs_pos = ram[ip++];
				var add_rhs = ram[add_rhs_pos];
				ram[ram[ip++]] = add_lhs + add_rhs;
				break;
			case 2:
				var mul_lhs = ram[ram[ip++]];
				var mul_rhs = ram[ram[ip++]];
				ram[ram[ip++]] = mul_lhs * mul_rhs;
				break;
			case 99:
				return;
		}
...
const String spec = @"--- Day 2: 1202 Program Alarm ---
